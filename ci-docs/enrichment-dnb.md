...
1. Жүктеп алу [csv үлгі файлымыз](https://c360devenrichment.blob.core.windows.net/mapping/DnBCIdatamapping.csv) ол аудиторияны түсіну өрістерін сәйкес Дан және Брэдстрит өрістеріне салыстыру үшін пайдаланылады. 
...
1. Таңдау **S3 мәліметтерін алыңыз**. Бұл ақпаратты қауіпсіз жерде сақтаңыз. Сізге қажет болады [байыту үшін қосылымды орнатыңыз](#configure-a-connection-for-dun--bradstreet) аудитория түсініктерінде. 
